...
io.on('connect', socket => {
    ...
    socket.on("join room", (roomid, username) => {
        ...
        if (rooms[roomid] && rooms[roomid].length > 0) {
            ...
            socket.to(roomid).emit('message', `${username} görüşmeye katıldı.`, 'EHD', moment().format(
                "h:mm"
            ));
    ...
    socket.on('disconnect', () => {
        ...
        socket.to(socketroom[socket.id]).emit('message', `${socketname[socket.id]} görüşmeden ayrıldı.`, `EHD`, moment().format(
            "h:mm"
        ));
